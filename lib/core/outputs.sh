...
shellspec_output_EVALUATION() {
  shellspec_output_raw statement "tag:evaluation" "note:" "fail:" \
    "evaluation:$SHELLSPEC_EVALUATION"
}

shellspec_output_SKIP() {
  if [ "$SHELLSPEC_SKIP_REASON" ]; then
    shellspec_output_raw statement "tag:skip" "note:" "fail:" \
...
shellspec_output_PENDING() {
  shellspec_output_raw statement "tag:pending" "note:PENDING" "fail:" \
...
shellspec_output_MATCHED() {
  shellspec_output_raw statement "tag:good" "note:" "fail:" \
...
shellspec_output_SYNTAX_ERROR() {
  shellspec_output_raw statement "tag:bad" "note:SYNTAX ERROR" "fail:y" \
    "message:${SHELLSPEC_EXPECTATION:-}"
